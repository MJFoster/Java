...
//	private int cacheSize; //Size of the cache.
	...
	private class BTreeNode {  // Inner class of BTree
		 ...
		 * @param writeIndex - parent's offset location.
		 */
	 ...
	 * @param  - the disk location of new node's parent
...
//			this.cacheSize = cacheSize;
	...
	private void splitChild(BTreeNode parent, int i) {
		...
		if(!leftChildNode.leaf) {
			for (int j = degreeT; j <= leftChildNode.n; j++) {
				...
				//childNode.thisNodeOffset = leftChildNode.thisNodeOffset;
	 ...
	 * 
	 * @param - new BTreeObject to be inserted.
	 */
	 ...
	 * @return BTreeNode - Node from secondary storage at specified offset.
	 */
	 ...
	 */
	public boolean checkEmpty(int rootOffset){
...
//				str += "Root Idx: " + rootOffset + ", Node Idx: " + node.thisNodeOffset + ", # keys: " + node.n + ", Parent Idx: " + node.parentPtr + ", leaf: " + node.leaf + " ";  // TODO:  Remove when done testing
	...
	/**
	 * Cache for the Btree cache option.
	 *
	 */
	...
	public class Cache {
		...
		/**
		 * get a node from the cache that is a certain offset.
		 * @param offset
		 * @return BTreeNode
		 */
		BTreeNode getNode(int offset) {
