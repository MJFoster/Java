...
public class IUDoubleLinkedList<T> implements IndexedUnorderedList<T> {
	 ...
	 * 
	 * When remove() completes:
	 * 	 If 1st element removed, previous & current BOTH -> head
	 *   If last element removed, previous -> tail, current -> null
	 */ 
	public T remove(int index) {
		...
		if(index < 0  ||  index >= size())     // out of bounds index
		...
		else if (size() == 1) { // reset head, tail, current, previous.
			...
			previous = head;
				...
				for (int i = 0; i < index; i++) {  // iterate until current -> index, previous is prior
					previous = current;
					current = current.getNext();
				}
	...
	// *************************  PRIVATE INNER CLASS  *************************
		...
		private class IUDLLIterator implements ListIterator<T> {
			...
			private DLLNode<T> lastReturned;   // node iterated over by prev or next
			...
			@Override
			public int nextIndex() {  // rear of list
				if(cursor == null)
					return size();
				else
					return itrIndex + 1;
			...
			public T previous() throws ConcurrentModificationException {
				...
				if( (size() == 0)  ||  (cursor == head  && size() > 1))     // empty list OR At front of list size > 1
					throw new NoSuchElementException("IUDLLIterator - next()");						
				else {
					// 1 element list OR iterating in the middle
					if (size() == 1)
						lastReturned = cursor;
					else {
						lastReturned = cursor.getPrev();      // save node for 'set' method
...
//				if (itrIndex < 1)  // boundary cases: empty list, or 1 element
//					return itrIndex;
//				else
//					return itrIndex - 1;
				return itrIndex;
			...
			public void remove() {
				...
				} else  // neither next() nor previous() have been called
